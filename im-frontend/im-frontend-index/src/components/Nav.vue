<template>
  <div class="tool">
    ...
    <div class="tool-avatar">
      <div class="tool-avatar-img" @click="showUserInfo('showUserModal')">
        <a-tooltip v-if="user" placement="left" arrow-point-at-center :title="user.username">
          <img :src="user.avatar ? user.avatar : randomAvatar" alt="" />
        </a-tooltip>
      </div>
    </div>
    <!-- 底部工具栏 -->
    ...
    <a v-if="isDemo" href="https://github.com/peanut996-im" target="_blank" class="tool-github icon"><a-icon type="github" /></a>
    ...
    <a-modal title="用户信息" :visible="showUserModal" footer="" @cancel="showUserModal = false">
      <div class="tool-user">
        ...
        >
          <a-avatar :src="user.avatar ? user.avatar : randomAvatar" class="img" :size="120"></a-avatar>
        ...
        <div class="tool-user-info">
          ...
          <a-input class="tool-user-input" v-model="account" placeholder="请输入新用户名"></a-input>
...
import { DEFAULT_BACKGROUND, getRandomAvatar } from '@/common/index';
...
export default class Tool extends Vue {
  ...
  // defaultAvatar: any = require('../assets/favicon.png');
  ...
  async changePassword() {
    ...
    const cipher = sha1(this.password).toUpperCase();
    this.$emit('updateUser', {
      uid: this.user.uid,
      password: cipher,
    });
  ...
  async handleUpload() {
    ...
    userCopy.avatar = result.url;
    this.setUser(userCopy);
    this.setUserGather(userCopy);
    this.$emit('updateUser', {
      uid: this.user.uid,
